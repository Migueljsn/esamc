...
function buildWhatsAppUrl(data: FormData) {
  ...
  const msg = `Olá! Me chamo ${data.nome} 👋\n\nAcabei de me cadastrar no site da Escola Santa Angélica para matricular ${data.nome_aluno || "meu(minha) filho(a)"}.\n\n🎓 Série pretendida: ${data.serie}\n📍 Bairro: ${data.bairro}\n📆 Quando pretende matricular: ${data.quando_matricular}`;
...
const QUANDO = [
  { label: "Este ano letivo", emoji: "🔥", sub: "Quero matricular agora" },
  { label: "Próximo ano letivo", emoji: "📅", sub: "Estou me planejando" },
  { label: "Ainda pesquisando", emoji: "🤔", sub: "Só quero conhecer a escola" },
];
...
export function LeadQuiz() {
  ...
  const validate = (): boolean => {
    switch (step) {
      ...
      case 6: if (form.nome_aluno.trim().length < 2) { setError("Informe o nome do aluno(a)."); return false; } break;
    }
    return true;
  };
  ...
  const next = async () => {
    ...
    if (step === 7) {
      setSubmitting(true);
      sendToSheets(form);
      trackLead({ serie: form.serie, nome_aluno: form.nome_aluno });
      setSubmitting(false);
      animateTo(8, "fwd");
      return;
    }
  ...
  const pick = (field: keyof FormData, value: string, autoNext = true) => {
    ...
    if (step === 7) {
      ...
      trackLead({ serie: updated.serie, nome_aluno: updated.nome_aluno });
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 1:
        return (
          <StepLayout
            ...
            hint="Você é o responsável pela matrícula — nossa equipe vai falar com você"
            error={error}
          >
            <input
              autoFocus
      ...
      // ── 5: série pretendida (SUGESTÃO COMERCIAL) ──────────────────────────
      case 5:
        return (
          <StepLayout
            question="Para qual série deseja matricular?"
          ...
          >
            <div className="grid grid-cols-1 gap-3 mt-2">
              {SERIES.map(s => (
                <button
                  key={s.label}
                  onClick={() => pick("serie", s.label)}
                  className={`${CARD_BTN} ${form.serie === s.label ? CARD_BTN_ACTIVE : ""}`}
                >
      ...
      case 6:
        return (
          <StepLayout
            question="Qual o nome do aluno(a)?"
            hint="Assim já deixamos tudo pronto para a matrícula"
            error={error}
          >
            <input
              autoFocus
              ...
              onChange={e => setForm(f => ({ ...f, nome_aluno: e.target.value }))}
              onKeyDown={e => e.key === "Enter" && next()}
      ...
      case 7:
        return (
          <StepLayout
            question="Quando pretende matricular?"
            hint="Isso nos ajuda a reservar a vaga"
            error={error}
          >
